...
<script>
	$(document).ready(function(){
        ...
        if ('WebSocket' in window){
			websocket = new WebSocket("ws://localhost:8080/webSocket/ws?uid="+${sessionScope.uid});
		}else if('MozWebSocket' in window){
			websocket = new MozWebSocket("ws://localhost:8080/webSocket/ws"+${sessionScope.uid});
		}else{
			websocket = new SockJS("http://localhost:8080/webSocket/ws/sockjs"+${sessionScope.uid});
		}
	    //var websocket = new WebSocket('ws://localhost:8080/Spring-websocket/ws');
	    websocket.onmessage = function(event) {//接收实时数据
